...
# ⚠️ ATENCIÓN: Se ha reordenado el flujo. El Teléfono ahora se pide en la Post-Venta.
MENU, VIVIENDA, PROBLEMA, CONSUMO, NOMBRE, BARRIO = range(6)
TELEFONO_POST, DIRECCION_POST = range(6, 8) # Nuevos estados para la post-venta
...
# ==========================================
# 🗓️ LÓGICA DE CALENDARIO Y HORARIOS 
# ==========================================
...
# ==========================================
# 🚀 FLUJO PRINCIPAL DEL BOT (PRE-VENTA)
# ==========================================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    teclado = [['☀️ Ver Productos y Soluciones'], ['📊 Solicitar Estudio Gratuito']]
    mensaje_bienvenida = "¡Hola! Bienvenido a *Solar Barrio* ☀️.\n\nEstamos encantados de ayudarte a dar el salto a la energía limpia, ahorrar en tu factura y protegerte de los apagones.\n\n¿En qué podemos asesorarte hoy?"
    await update.message.reply_text(mensaje_bienvenida, parse_mode='Markdown', reply_markup=ReplyKeyboardMarkup(teclado, one_time_keyboard=True, resize_keyboard=True))
    return MENU
...
¿Te gustaría un estudio personalizado sin compromiso?"""
        await update.message.reply_text(mensaje_kits, parse_mode='Markdown', reply_markup=ReplyKeyboardMarkup([['📊 Solicitar Estudio Gratuito']], resize_keyboard=True))
        return MENU
    elif "Solicitar" in opcion:
        await update.message.reply_text("¡Genial! Vamos a diseñar algo a tu medida. 🏠 Primero, ¿qué tipo de vivienda tienes?", reply_markup=ReplyKeyboardMarkup([['Casa independiente', 'Adosado / Chalet'], ['Piso / Apartamento', 'Negocio / Local']], resize_keyboard=True))
        return VIVIENDA
...
async def recibir_vivienda(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Entendido. ⚡ ¿Cuál dirías que es tu problema principal ahora mismo?", reply_markup=ReplyKeyboardMarkup([['Cortes de luz / Apagones', 'Facturas muy caras'], ['Ambas opciones']], resize_keyboard=True))
    return PROBLEMA
...
async def recibir_problema(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Tomamos nota. 💡 ¿Cuál es tu gasto mensual aproximado en electricidad?", reply_markup=ReplyKeyboardMarkup([['Menos de 50€', 'Entre 50€ y 100€'], ['Más de 100€', 'No lo sé seguro']], resize_keyboard=True))
    return CONSUMO
...
async def recibir_barrio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # ==========================================
    # 🤖 1. CÁLCULO DE PRESUPUESTO AUTOMÁTICO
    # ==========================================
